...
class PokemonDisplayElement:
    def __init__(self):
        ...
        self.primary_type_button = pokemon.Type.create_button(f"pokemon_display_element_type_primary_{self.uuid}")
        self.secondary_type_button = pokemon.Type.create_button(f"pokemon_display_element_type_secondary_{self.uuid}")
        self.attribute_rows = {}
        for attr_name in pokemon.Stats.ALL_ATTR_NAMES:
            self.attribute_rows[attr_name] = [gui.Text("", key=f"pokemon_display_element_attribute_name_{attr_name}_{self.uuid}", size=(12, 1)), gui.Text("", key=f"pokemon_display_element_attribute_value_{attr_name}_{self.uuid}", size=(3, 1)), pokemon.Stats.Attribute.create_graph(f"pokemon_display_element_attribute_bar_{attr_name}_{self.uuid}")]
